...
export default function DepartmentListPage({ apiBaseUrl }: DepartmentListPageProps) {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`${apiBaseUrl}/departments/${id}`, { method: 'DELETE' });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            ...
            {!loading && !error && filtered.length === 0 && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-600 font-medium">
                        {search ? 'No matching records found' : 'No departments yet'}
                    </p>
                    <p className="text-slate-400 text-sm mt-1">
                        {search ? 'Try a different search term' : 'Create your first department to get started'}
                    </p>
                    {!search && (
                        <a href={`/departments/new`} className="inline-block mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                            + Add Department
                        </a>
                    )}
                </div>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filtered.map((item, idx) => (
                                    <tr key={item.id} className={`hover:bg-slate-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                        ...
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.head_user_id != null ? String(item.head_user_id) : <span className="text-slate-300">—</span>}
                                        </td>
