...
export default function GradeListPage({ apiBaseUrl }: GradeListPageProps) {
    ...
    const handleDelete = async (id: number) => {
        if (!confirm('Permanently delete this grade?')) return;
        try {
            const res = await fetch(`${apiBaseUrl}/grades/${id}`, { method: 'DELETE' });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            ...
            {!loading && !error && filtered.length === 0 && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-600 font-medium">
                        {search ? 'No matching records found' : 'No grades yet'}
                    </p>
                    <p className="text-slate-400 text-sm mt-1">
                        {search ? 'Try a different search term' : 'Create your first grade to get started'}
                    </p>
                    {!search && (
                        <a href={`/grades/new`} className="inline-block mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                            + Add Grade
                        </a>
                    )}
                </div>
                                ...
                                {filtered.map((item, idx) => (
                                    <tr key={item.id} className={`hover:bg-slate-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                        ...
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.enrollment_id != null ? String(item.enrollment_id) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.score != null ? String(item.score) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.letter_grade != null ? String(item.letter_grade) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-600">
                                            {item.graded_at ? new Date(item.graded_at).toLocaleDateString() : '—'}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.graded_by != null ? String(item.graded_by) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <a
                                                    href={`/grades/${item.id}/edit`}
