...
interface PrerequisiteListPageProps {
    apiBaseUrl: string;
}

export default function PrerequisiteListPage({ apiBaseUrl }: PrerequisiteListPageProps) {
    ...
    const fetchItems = async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`${apiBaseUrl}/prerequisites`);
    ...
    const handleDelete = async (id: number) => {
        if (!confirm('Permanently delete this prerequisite?')) return;
        try {
            const res = await fetch(`${apiBaseUrl}/prerequisites/${id}`, { method: 'DELETE' });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    return (
        <div className="p-6 max-w-7xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Prerequisites</h1>
                    <p className="text-sm text-slate-500 mt-0.5">
                        {filtered.length} of {items.length} record{items.length !== 1 ? 's' : ''}
                    </p>
                </div>
                <a
                    href={`/prerequisites/new`}
            ...
            {loading && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-500 text-sm">Loading prerequisites...</p>
                </div>
            )}
            ...
            {!loading && !error && filtered.length === 0 && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-600 font-medium">
                        {search ? 'No matching records found' : 'No prerequisites yet'}
                    </p>
                    <p className="text-slate-400 text-sm mt-1">
                        {search ? 'Try a different search term' : 'Create your first prerequisite to get started'}
                    </p>
                    {!search && (
                        <a href={`/prerequisites/new`} className="inline-block mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                            + Add Prerequisite
                        </a>
                    )}
                </div>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filtered.map((item, idx) => (
                                    <tr key={item.id} className={`hover:bg-slate-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                        ...
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.prerequisite_course_id != null ? String(item.prerequisite_course_id) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3">
                                            <span className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${item.is_mandatory ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                                        ...
                                        <td className="px-5 py-3 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <a
                                                    href={`/prerequisites/${item.id}/edit`}
