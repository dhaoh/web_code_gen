...
export default function CourseScheduleFormPage() {
    ...
    useEffect(() => {
        if (isEdit) {
            fetch(`http://localhost:8000/courseschedules/${id}`)
                .then(res => {
                ...
                .then(data => {
                    ...
                    fd.schedule_id = data.schedule_id ?? formData.schedule_id;
                    setFormData(fd);
                })
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        if (e.target.name === 'schedule_id') value = value === '' ? null : Number(value);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        const url = isEdit
            ? `http://localhost:8000/courseschedules/${id}`
            : `http://localhost:8000/courseschedules`;
    ...
    return (
        <div className="p-6 max-w-2xl">
            ...
            <div className="mb-6">
                <button
                    onClick={() => navigate('/courseschedules')}
                ...
                >
                    ...
                    Back to courseschedules
                </button>
                <h1 className="text-2xl font-bold text-slate-900">
                    {isEdit ? 'Edit' : 'Create New'} Courseschedule
                </h1>
            ...
            <form onSubmit={handleSubmit}>
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                        <h2 className="text-sm font-semibold text-slate-700">Courseschedule Details</h2>
                    </div>
                        ...
                        >
                            {saving ? (
                                ...
                                <>
                                    ...
                                    {isEdit ? 'Save Changes' : 'Create Courseschedule'}
                                </>
                            )}
                        </button>
                        <button
