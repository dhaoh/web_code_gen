...
export default function AssignmentListPage({ apiBaseUrl }: AssignmentListPageProps) {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`${apiBaseUrl}/assignments/${id}`, { method: 'DELETE' });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            ...
            {!loading && !error && filtered.length === 0 && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-600 font-medium">
                        {search ? 'No matching records found' : 'No assignments yet'}
                    </p>
                    <p className="text-slate-400 text-sm mt-1">
                        {search ? 'Try a different search term' : 'Create your first assignment to get started'}
                    </p>
                    {!search && (
                        <a href={`/assignments/new`} className="inline-block mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                            + Add Assignment
                        </a>
                    )}
                </div>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {filtered.map((item, idx) => (
                                    <tr key={item.id} className={`hover:bg-slate-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                        ...
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.max_score != null ? String(item.max_score) : <span className="text-slate-300">—</span>}
                                        </td>
