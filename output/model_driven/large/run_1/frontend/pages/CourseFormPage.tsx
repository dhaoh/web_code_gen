...
export default function CourseFormPage() {
        ...
        if (isEdit) {
            fetch(`http://localhost:8000/courses/${id}`)
                ...
                .then(data => {
                    ...
                    fd.teacher_id = data.teacher_id ?? formData.teacher_id;
                    fd.classroom_id = data.classroom_id ?? formData.classroom_id;
                    fd.semester = data.semester ?? formData.semester;
                    setFormData(fd);
                })
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        if (e.target.name === 'teacher_id') value = value === '' ? null : Number(value);
        if (e.target.name === 'classroom_id') value = value === '' ? null : Number(value);
