...
export default function MajorListPage({ apiBaseUrl }: MajorListPageProps) {
    ...
    const fetchItems = async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`${apiBaseUrl}/majors`);
    ...
    const handleDelete = async (id: number) => {
        if (!confirm('Permanently delete this major?')) return;
        try {
            const res = await fetch(`${apiBaseUrl}/majors/${id}`, { method: 'DELETE' });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
        ...
        <div className="p-6 max-w-7xl">
            ...
            {loading && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-500 text-sm">Loading majors...</p>
                </div>
            )}
            ...
            {!loading && !error && filtered.length === 0 && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-600 font-medium">
                        {search ? 'No matching records found' : 'No majors yet'}
                    </p>
                    <p className="text-slate-400 text-sm mt-1">
                        {search ? 'Try a different search term' : 'Create your first major to get started'}
                    </p>
                    {!search && (
                        <a href={`/majors/new`} className="inline-block mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                            + Add Major
                        </a>
                    )}
                </div>
                                ...
                                {filtered.map((item, idx) => (
                                    <tr key={item.id} className={`hover:bg-slate-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                        ...
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.name != null ? String(item.name) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                        ...
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.total_credits_required != null ? String(item.total_credits_required) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <a
                                                    href={`/majors/${item.id}/edit`}
