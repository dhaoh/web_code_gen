...
export default function CourseListPage({ apiBaseUrl }: CourseListPageProps) {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`${apiBaseUrl}/courses/${id}`, { method: 'DELETE' });
            if (!res.ok) throw new Error(`HTTP ${res.status}`);
            ...
            {!loading && !error && filtered.length === 0 && (
                <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
                    ...
                    <p className="text-slate-600 font-medium">
                        {search ? 'No matching records found' : 'No courses yet'}
                    </p>
                    <p className="text-slate-400 text-sm mt-1">
                        {search ? 'Try a different search term' : 'Create your first course to get started'}
                    </p>
                    {!search && (
                        <a href={`/courses/new`} className="inline-block mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
                            + Add Course
                        </a>
                    )}
                </div>
                                ...
                                {filtered.map((item, idx) => (
                                    <tr key={item.id} className={`hover:bg-slate-50 transition ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                                        ...
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.title != null ? String(item.title) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.code != null ? String(item.code) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.description != null ? String(item.description) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.capacity != null ? String(item.capacity) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.credits != null ? String(item.credits) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.department_id != null ? String(item.department_id) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.teacher_id != null ? String(item.teacher_id) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.classroom_id != null ? String(item.classroom_id) : <span className="text-slate-300">—</span>}
                                        </td>
                                        <td className="px-5 py-3 text-sm text-slate-700">
                                            {item.semester != null ? String(item.semester) : <span className="text-slate-300">—</span>}
                                        </td>
