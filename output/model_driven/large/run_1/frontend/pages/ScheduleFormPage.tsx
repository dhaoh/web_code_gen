...
export default function ScheduleFormPage() {
    ...
    useEffect(() => {
        if (isEdit) {
            fetch(`http://localhost:8000/schedules/${id}`)
                ...
                .then(data => {
                    const fd: any = {};
                    fd.day_of_week = data.day_of_week ?? formData.day_of_week;
                    fd.start_time = data.start_time ?? formData.start_time;
                    fd.end_time = data.end_time ?? formData.end_time;
                    setFormData(fd);
                })
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        if (e.target.name === 'day_of_week') value = value === '' ? null : Number(value);
        ...
        <div className="p-6 max-w-2xl">
            ...
            <div className="mb-6">
                ...
                <h1 className="text-2xl font-bold text-slate-900">
                    {isEdit ? 'Edit' : 'Create New'} Schedule
                </h1>
            ...
            <form onSubmit={handleSubmit}>
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                        <h2 className="text-sm font-semibold text-slate-700">Schedule Details</h2>
                    </div>
