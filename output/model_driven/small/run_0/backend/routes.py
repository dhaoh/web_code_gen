...
def update_student(item_id: int, data: StudentUpdate, session: Session = Depends(get_session)):
    ...
    if "email" in data.model_dump(exclude_unset=True):
...
def delete_student(item_id: int, session: Session = Depends(get_session)):
    ...
    # No additional cascade restrictions required
    session.delete(item)
    session.commit()
...
def create_course(data: CourseCreate, session: Session = Depends(get_session)):
    ...
    # No duplicate or enrollment rules apply to course creation
...
def update_course(item_id: int, data: CourseUpdate, session: Session = Depends(get_session)):
    ...
    # No special business rules for course update
    for key, value in data.model_dump(exclude_unset=True).items():
        setattr(item, key, value)
    session.commit()
    session.refresh(item)
...
def update_enrollment(item_id: int, data: EnrollmentUpdate, session: Session = Depends(get_session)):
    ...
    for key, value in data.model_dump(exclude_unset=True).items():
        setattr(item, key, value)
    # Re-check business rules if student or course changed
    changed_fields = data.model_dump(exclude_unset=True)
    if "student_id" in changed_fields or "course_id" in changed_fields:
        ...
        if not course:
            raise HTTPException(status_code=404, detail="Course not found")
        # Duplicate check (excluding current enrollment)
        duplicate = session.query(Enrollment).filter(
        ...
        if duplicate:
            raise HTTPException(status_code=409, detail="Student is already enrolled in this course")
        # Capacity check only if course changed
        if "course_id" in changed_fields:
